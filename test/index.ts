...
if (process.env["CID"]?.length == 0) throw new Error("CID is empty !")
...
describe("Test Contract KaveuERC721", function () {
  ...
  before(async () => {
    ...
    // https://mumbai.polygonscan.com/address/0xf46f27ca5D858103e7ad5D7dFd4556786010d2f8#code
    kaveu = await ethers.getContractAt("KaveuERC721", "0xf46f27ca5D858103e7ad5D7dFd4556786010d2f8", sgn1)
  ...
  describe("To withdraw() section", async () => {
    ...
    it("all tokenURI", async () => {
      // baseURI is uri_
      expect(await await kaveu.getTokenURIs()).to.have.length(15)
    })
    ...
    it("increaseClaws", async () => {
      ...
      // the test makes with gwei unit not ether
      const nextPriceClaw = nextTotalClaw.mul(utils.parseUnits("5.7614", "gwei"))
